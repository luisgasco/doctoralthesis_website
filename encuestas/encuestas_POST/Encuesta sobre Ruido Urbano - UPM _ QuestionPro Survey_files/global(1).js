...
function openPreviewReports(loc) {
    var winl = (screen.width-800)/2;
    var wint = (screen.height-640)/2;

    myRemote = launch(loc,"FreeSurveys1","height=640,width=1220,channelmode=0,dependent=0,directories=0,fullscreen=0,location=0,menubar=0,resizable=1,scrollbars=1,status=0,toolbar=0,top="+wint + ",left=" + winl,"Help");
    window.myRemote.focus();
}
...
function helpWindow(loc) {
    ...
    myRemote = launch(loc,"FreeSurveys3","height=700,width=1120,channelmode=0,dependent=0,directories=0,fullscreen=0,location=0,menubar=0,resizable=1,scrollbars=1,status=1,toolbar=1,location=1,top=" + wint + ",left=" + winl,"Help");
    window.myRemote.focus();
}
...
function openWindow(loc, winName) {
    var winl = (screen.width-800)/2;
    var wint = (screen.height-640)/2;

    myRemote = launch(loc,winName,"height=640,width=1130,channelmode=0,dependent=0,directories=0,fullscreen=0,location=0,menubar=0,resizable=1,scrollbars=1,status=0,toolbar=0,top="+wint + ",left=" + winl,"Help");
    window.myRemote.focus();
}
    ...
    init: function() {
        if (Bubble.bubbleDiv == null) {
            ...
            var html = '<table cellpadding="0" cellspacing="0" border="0"><tr>'
                ...
                + Bubble.imagePath + 'bubble_left.gif" height="2" width="2" class="bubble-left">'
                + '</td><td valign="top"'
                ...
                + Bubble.imagePath+ 'bubble_tail.gif" height="11" width="10" class="bubble-tail">'
                + '</td><td style="width:9px" '
                + 'valign="top"><img src="'
                + Bubble.imagePath + 'bubble_bg.gif" width="9" height="2"  class="bubble-bg"></td>'
                + '<td style="width:2px" valign="top"><img src="'
                + Bubble.imagePath + 'bubble_right.gif" height="2" width="2" class="bubble-right">'
                + '</td></tr>'
                + '</table></td></tr></table>';
...
function validateZip(value, divID, ajaxProcessorURL, zipMaxLength,zipMinLength) {
    if (value != null &&  value.length >= zipMinLength && value.length <=zipMaxLength) {
...
function isSafari() {
    var browserName= navigator.userAgent.toLowerCase();
    return (browserName && browserName.indexOf("safari") > -1 && browserName.indexOf('chrome') < 0);
}
...
function ajaxRequestV2(params) {
    ...
    if (overlaySpinner) {
        element.innerHTML = '<div  align="center" style="position:absolute;' +
            ...
            '<div style="margin-top:' + (element.clientHeight/2) + 'px;"><img src="/images/loading-c.gif" > &nbsp; <span style="color:#666666;position:relative;top:-5px;">Loading...</span></div></div>' +
            element.innerHTML;
    } else if (!silent) {
        (spinnerDiv ? spinnerDiv : element).innerHTML = params.spinner ? params.spinner : '<div ' +
            ' align="center">' +
            '<img src="/images/' + (spinnerImage ? spinnerImage : "loading-c.gif") + '"> &nbsp; <span style="color:' + (loadingTextColor ? loadingTextColor : '#666666') + ';position:relative;top:-5px;">Loading...</span></div>';
    }
...
function ajaxToolTipPopup(params) {
    var elementID = params.elementID;
    var divID = params.divID;
    var url = params.url;
    var focusfield =params.autoFocusField;
    var autofocus = params.autofocus;
    var effect = params.animation;
    var container = document.getElementById('inlineDiv');
    ...
    r.onreadystatechange = function () {
        ...
        if (responseText.indexOf("ajax-eval-code") > 0) {
            var ary = responseText.split("<ajax-eval-code>");
            $('#' + elementID).html(ary[0]);
            eval(ary[1].split("</ajax-eval-code>")[0]);
        } else if (responseText.indexOf("//JavaScript") > 0) {
            eval(responseText);
        } else {
            tooltip.pop(element, responseText, {position:1, offsetX:5, offsetY:5, effect:effect, sticky: 1});
        }
    }
}
...
function ajaxLinkForControlWYSIWYG(element, elementID, url, append) {
    var r = getAjaxRequest(url, element);
    r.onreadystatechange = function () {
        if (r.readyState != 4) {
            return;
        }
        $("#" + elementID).redactor('set',(stringTrim(r.responseText) + (append ? element.value.replace(/Please wait\.\.\./i,""): "")));
    }

    $("#" + elementID).redactor('set', 'Please wait...' + element.value);
}
...
LightboxInternal.JS = {
    ...
    openNonProfit: function() {
        ...
        div.innerHTML = "<div id='imageLBDiv'><div class='chatCloseDiv' onclick=\"LightboxInternal.JS.unblockBg();\" style='float:right;position:relative;left:22px;top:-20px;cursor:pointer;'>" +
            "<img src='/images/qphome/chat-Close.png'></div><div class=\"nonprofit_blob\" style=\"top:-35px;height:220px;\"><div><div class=\"r7_c2_c1 nonprofit_blob_c1\" > Get Started! </div><div class=\"nonprofit_blob_c3\"><div>1. Signup for a free account with QuestionPro. </div><div>2. Add a short statement to your website  homepage that acknowledges/thanks QuestionPro as the sponsor of your survey  software. This text should be 1-3 sentences long  and may include a link to http://www.questionpro.com.</div><div style=\"top:5px;\">3. After adding the statement to your homepage, please send an email to <a href='mailto:non-profit@questionpro.com'>non-profit@questionpro.com</a> <br/>one of our representatives will get back to you shortly to upgrade your account for free!</div></div></div></div></div>";
        var overlayDiv = document.getElementById("lightboxOverlay");
        overlayDiv.style.top = 0;
        overlayDiv.onclick = function() {
            LightboxInternal.JS.unblockBg();
...
function getSalesChatWindow() {
    return  '<div class="chatBoxO" id="SalesChatDiv">'+
        '<div class="chatCloseDiv" onclick="hideDiv(\'SalesChatDiv\');markNoSalesChat();" style="float:right;position:relative;left:15px;top:-15px;cursor:pointer;z-index:1;">' +
        '<img src="/images/qphome/chat-Close.png">' +
        '</div>' +
        '<div class="chatBoxHeaderO">'+
        ...
        '</div>'+
        '<div id="alertMsgPopup" style="display:none;"></div>'+
        '<div style="margin-bottom:5px; text-align:right;">'+
        '<input type="button" class="startChat" value="Start Chat" onclick="formValidatePopup(\'emailText1\');" />'+
        '</div>'+
        '</form>'+
        '</div>'+
...
function swapImg() {
    var image = new Array ();
    image[0] = "/images/chatbox/jemini-patel.png";
    image[1] = "/images/chatbox/kartik-bhat.png";
    image[2] = "/images/chatbox/naeem-shaikh.png";
    image[3] = "/images/chatbox/noel-fernandes.png";
    image[4] = "/images/chatbox/roshan-vig.png";
    image[5] = "/images/chatbox/sylvina-roy.png";
    var size = image.length
    var x = Math.floor(size*Math.random())
    var y = image[x].replace('/images/chatbox/', '').replace('-', ' ').replace('.png', '');
...
function formValidatePopup(inputid){
    ...
    if(textval.value.trim().length > 0){
        ...
        $('#alertMsgPopup').show();
        $('#alertMsgPopup').text('Please fill out this field.');
        textval.focus();}
}
...
function validateEmail(emailinput) {
    ...
    var valid = emailReg.test(emailId.value);
    if (!valid) {
        return false;
    } else {
        return true;
...
function defaultSelect(id, condition){
    if (condition) {
...
function randomAgent (imgClass, imgFormat, agentName){
    var image = new Array ();
    image[0] = '/images/chatbox/jemini-patel.'+imgFormat;
    image[1] = '/images/chatbox/kartik-bhat.'+imgFormat;
    image[2] = '/images/chatbox/naeem-shaikh.'+imgFormat;
    image[3] = '/images/chatbox/noel-fernandes.'+imgFormat;
    image[4] = '/images/chatbox/roshan-vig.'+imgFormat;
    image[5] = '/images/chatbox/sylvina-roy.'+imgFormat;
    var size = image.length;

    var x = Math.floor(size*Math.random());
    var imagePath = image[x];
    var y = imagePath.replace('/images/chatbox/', '').replace('-', ' ').replace('.'+imgFormat, '');
    $(imgClass).attr('src',imagePath);
    $(agentName).html(y);
    return imagePath;
}

function hasThirdPartyCookiesDisabled() {
